...
void procesarDatos(long bytesRecibidos) {
  // Definir el umbral de bytes para activar el LED rojo
  long umbralRojo = 1000000000000; // Ajusta este valor según las especificaciones de senior jaime
  
  // Verificar si la cantidad de bytes recibidos alcanza el umbral para el LED rojo
  if (bytesRecibidos >= umbralRojo) {
    // Encender el LED rojo y apagar el LED azul
    digitalWrite(LED_ROJO_PIN, HIGH);
    digitalWrite(LED_AZUL_PIN, LOW);
  } else {
    // Apagar el LED rojo y encender el LED azul
